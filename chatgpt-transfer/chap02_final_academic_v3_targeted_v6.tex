...
\end{equation}
其中，$C_{10}$ 和 $C_{01}$ 为材料常数，$\kappa$ 为体积模量。前两项描述等容变形，第三项限制体积变化。对于近似不可压缩材料，$J$ 接近 $1$\cite{mooney1940large,rivlin1948large}。为避免采用应变能对张量的形式导数，定义等容左 Cauchy--Green 变形张量分量为 $\bar B_{ij}=J^{-2/3}\sum_{k=1}^{3}F_{ik}F_{jk}$，则弹性 Cauchy 应力分量为
\begin{equation}
\begin{aligned}
\sigma_{\mathrm e,ij}
={}&
...
\end{equation}
其中，$\sigma_{11}$、$\sigma_{22}$ 和 $\sigma_{33}$ 为正应力分量，$\sigma_{12}$、$\sigma_{23}$ 和 $\sigma_{31}$ 为剪应力分量。固体总弹性应变能为
\begin{equation}
W_{\mathrm s}(t)
=
