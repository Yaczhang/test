...
微通道细胞受限变形过程中，胞内外介质均视为不可压缩牛顿流体。细胞边界在数值离散中按具有有限厚度的超弹性固体层处理，其内外表面分别与胞内流体和胞外流体构成流固界面。入口与出口之间的压力差驱动细胞进入收缩段，细胞依次经历横向压缩、轴向伸长和离开约束后的形态恢复。该模型不包含外部气液自由界面，数值求解输出细胞轮廓、质心位置和运动速度，预先给定的材料参数作为过程级标签。

含细胞复合液滴由周围气相、外部液滴液相和内部细胞组成。液滴下降接触固壁后经历接触、铺展、最大铺展和回缩，内部细胞在非定常压力、黏性剪切及自身材料恢复作用下发生变形和迁移。该模型在流固耦合基础上进一步引入相场气液界面、表面张力和壁面润湿条件。数值求解输出液滴外轮廓、铺展因子、接触线位置和名义动态接触角，以及内部细胞的轮廓、质心运动、等效应力和弹性应变能。弹性模量和黏弹性特征时间属于预先设置的材料参数，不作为正问题求解器的输出量。

图~\ref{fig:ch2_two_models}给出了两类研究对象及其共同建模主线。二者共享流体方程、细胞材料模型、流固耦合条件和有限元求解框架，差异主要体现在流体域组成、界面类型、驱动方式、细胞几何表示和观测量。

\begin{figure}[htbp]
  \centering
  \input{figures/chap02/fig2_01_two_models_final.tex}
...
主要观测 & 细胞轮廓、质心和速度 & 内外轮廓、铺展、名义动态接触角及细胞响应 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \input{figures/chap02/fig2_02_multiphysics_domains_final.tex}
...
设润湿边界为 $\Gamma_{\mathrm b}$，$\boldsymbol{n}_{\Omega}$ 为流体计算域在该边界处的外法向量，平衡接触角 $\theta_{\mathrm e}$ 在液相内部测量。与式~\eqref{eq:ch2_phase_free_energy}一致的壁面自由能密度写为
\begin{equation}
f_{\mathrm w}(\varphi)
=
...
=0,
\qquad
\boldsymbol{x}\in\Gamma_{\mathrm b},
...
\end{equation}
对于固体壁面，$\Gamma_{\mathrm b}=\Gamma_{\mathrm w}$，平衡接触角取相应工况设定值。含细胞复合液滴模型还在内部细胞外表面采用固定相场润湿参数，该参数在全部工况中保持不变，不作为参数分析对象。令 $\boldsymbol{n}_{\mathrm w}=-\boldsymbol{n}_{\Omega}$ 表示由固壁指向流体区域的单位法向量，气液界面单位法向量 $\boldsymbol{n}_{\mathrm{lg}}$ 由气相指向液相，则平衡状态下有
\begin{equation}
\cos\theta_{\mathrm e}
=
...
\end{equation}
上述法向取向同时用于后续接触角计算，避免平衡接触角与后处理角度采用不同约定\cite{deGennes1985wetting,bonn2009wetting,snoeijer2013moving}。
...
\alpha_{\mathrm s}
=
\left(\frac{d_{\mathrm s,0}}{D_0}\right)^3
=
\Lambda^3,
\label{eq:ch2_volume_fraction}
\end{equation}
其中，$d_{\mathrm s,0}$ 为内部细胞初始直径。
...
=
\frac{1}{2}
\left(\boldsymbol{C}_{\mathrm{CG}}-\boldsymbol{I}\right).
\label{eq:ch2_strain_tensors}
\end{equation}
$\boldsymbol{C}_{\mathrm{CG}}$ 的三个不变量为
\begin{equation}
I_1=\operatorname{tr}\boldsymbol{C}_{\mathrm{CG}},
\qquad
I_2=\frac{1}{2}
\left[
\left(\operatorname{tr}\boldsymbol{C}_{\mathrm{CG}}\right)^2
-
\operatorname{tr}\left(\boldsymbol{C}_{\mathrm{CG}}^2\right)
\right],
\qquad
I_3=\det\boldsymbol{C}_{\mathrm{CG}}=J^2,
...
\boldsymbol{S}
=
2\frac{\partial W_{\mathrm{MR}}}{\partial\boldsymbol{C}_{\mathrm{CG}}},
\qquad
...
\end{equation}
式~\eqref{eq:ch2_mr_modulus_relation}为莫尼里夫林模型的小变形理论关系。第三章中材料控制参数与弹性模量标签之间的具体换算由该章给出的 $C_{10}$ 和 $C_{01}$ 设置确定。

微通道模型中的固体域表示具有有限厚度的连续等效细胞膜层，膜层内外均为流体区域。含细胞复合液滴模型中的固体域表示具有有限体积的内部细胞。两类模型均采用上述有限变形框架，具体几何参数和材料参数分别在第三章和第四章给出。
...
\end{equation}
其中，$\eta_{\mathrm s}$ 为本文数值模型中的等效黏性系数。与第四章的参数设置一致，黏弹性特征时间定义为
\begin{equation}
\tau_{\mathrm v}
=
...
\end{equation}
该定义用于第四章全部黏弹性参数工况。开尔文沃伊特模型由弹性部分和黏性部分并联组成，可描述加载过程中的速率相关响应和卸载后的恢复\cite{christensen1982theory,lakes1998viscoelastic}。
...
\begin{figure}[htbp]
  \centering
  \input{figures/chap02/fig2_03_ale_mesh_final.tex}
...
\end{aligned}
\end{equation}
式~\eqref{eq:ch2_weak_phase_auxiliary}中的最后一项由式~\eqref{eq:ch2_wetting_bc}得到，使壁面自由能、平衡接触角和有限元弱形式采用同一润湿模型。
...
\end{equation}
其中，$N_i$ 为形函数，$y_i$ 为节点自由度，$N_{\mathrm n}$ 为单元节点数。流体速度、压力、相场变量、辅助变量、固体位移和网格位移分别在相应有限元空间中展开。速度和压力插值需满足不可压缩流动的稳定性要求，相场变量和辅助变量采用连续插值。两类模型的具体单元阶次、网格尺度、局部加密范围和求解器设置在对应研究章节中给出，避免将不同计算域的离散参数混写为同一设置。
...
\end{equation}
其中，$\boldsymbol{K}_{\mathrm T}^{(m)}=\partial\boldsymbol{r}/\partial\boldsymbol{Y}$ 为切线矩阵，$m$ 为非线性迭代次数。时间步根据界面运动、固体变形速度和收敛状态控制，具体范围及无关性分析分别在第三章和第四章给出。
...
\end{equation}
其中，$N_t$ 为比较时刻数。除完整时间历程外，还比较峰值、峰值时刻和关键时刻的界面形态。模型验证、网格设置和时间步设置分别在第三章和第四章结合具体计算域给出。

\section{形态响应的统一表征}

数值求解得到流场、固体位移和界面演化后，需要提取与成像观测相对应的形态和运动信息。两类问题均采用内部细胞轮廓和质心位置描述细胞响应。微通道问题进一步使用傅里叶系数压缩完整轮廓序列。含细胞复合液滴问题还需提取外部液滴铺展宽度、接触线位置和名义动态接触角。
...
\end{equation}
为去除整体平移对形态表示的影响，轮廓点转换为相对于质心的坐标
\begin{equation}
...
第三章点坐标重构直接使用样本文件中保存的归一化坐标。设样本生成阶段采用的固定坐标变换为 $\mathcal{N}_{\mathrm{coord}}$，则网络使用的坐标写为
\begin{equation}
\boldsymbol{x}_{i,\ell}^{\mathrm n}
=
\mathcal{N}_{\mathrm{coord}}
\left(\boldsymbol{x}_{i,\ell}^{\mathrm{phys}}\right).
\label{eq:ch2_stored_coordinate_normalization}
\end{equation}
训练输入、预测目标、坐标损失、坐标扰动和平均点误差均在同一数据尺度上计算。由于原始样本档案未单独保存 $\mathcal{N}_{\mathrm{coord}}$ 的物理尺度参数，点坐标误差作为无量纲数据尺度误差报告，不换算为长度单位。闭合轮廓的采样点数、起始点和排列方向在同一任务中保持一致。预测量使用帽号，质心平移后的量使用波浪号，样本文件中的归一化坐标使用上标 $\mathrm n$。
...
\end{equation}
傅里叶表示能够降低点坐标维数，并通过阶数控制整体形态与局部细节之间的平衡\cite{kuhl1982elliptic}。
...
\end{equation}
圆形轮廓满足 $\mathcal{C}=1$ 和 $D_{\mathrm c}=0$。全文将 $\mathcal{C}$ 称为圆度，将 $D_{\mathrm c}$ 称为圆度偏差。若采用长短轴定义的变形指数或长宽比，在相应研究章节中单独给出公式。

\subsection{质心运动、液滴铺展与名义动态接触角}
...
接触线处气液界面与壁面之间位于液相内部一侧的夹角记为 $\theta_{\mathrm d}$。取 $\boldsymbol{n}_{\mathrm w}$ 由固壁指向流体区域，$\boldsymbol{n}_{\mathrm{lg}}$ 由气相指向液相，则几何关系为
\begin{equation}
\theta_{\mathrm d}(t)
=
\arccos
...
\end{equation}
第四章采用统一的图像分割、界面提取和局部曲线拟合流程计算该角度。由于原始后处理记录未单独保存二值化阈值、拟合点数和物理拟合窗口长度，全文将该流程得到的角度称为名义动态接触角。该量用于描述相同处理条件下的阶段变化和不同工况之间的相对差异，不作为独立标定的绝对接触角。具体图像尺寸、接触点判定和曲线拟合流程见第四章。

\section{形态驱动的力学参数反演}

\subsection{正问题、观测过程与参数反演}

图~\ref{fig:ch2_inverse_framework}给出了离线样本生成与在线参数反演之间的关系。几何参数、流动参数和边界条件作为已知工况输入。离线阶段在给定材料参数下求解正问题并生成带标签样本，在线阶段根据观测形态估计待识别的力学参数。

\begin{figure}[htbp]
  \centering
  \input{figures/chap02/fig2_04_observation_inverse_framework_final.tex}
  \caption{形态响应生成、观测处理与力学参数反演的统一框架。}
  \label{fig:ch2_inverse_framework}
\end{figure}

设 $\boldsymbol{m}$ 为材料参数，$\boldsymbol{q}$ 为已知的流动参数、几何参数和边界条件。给定 $\boldsymbol{m}$ 和 $\boldsymbol{q}$ 后，完整状态或形态响应表示为
\begin{equation}
\mathcal{Y}
=
...
\end{equation}
其中，$\mathcal{I}_{\boldsymbol{\theta}}$ 为训练得到的反演映射，$\boldsymbol{\theta}$ 为网络参数，$\widehat{\boldsymbol{m}}$ 为预测结果。数据驱动模型学习的是给定数值模型、参数范围和观测方式下的反演关系。

数值样本按完整计算过程划分为训练集、验证集和测试集。同一过程中的不同时间帧、稀疏组合和噪声版本只进入一个数据集合。需要标准化的连续变量采用训练集统计量进行处理
\begin{equation}
x^{*}
=
...
\end{equation}
其中，$\mu_x^{\mathrm{tr}}$ 和 $s_x^{\mathrm{tr}}$ 分别为训练集均值和标准差。验证集和测试集使用训练集确定的标准化参数，预测结果评价前还原到原始物理单位。式~\eqref{eq:ch2_standardization}用于一般连续特征，不替代式~\eqref{eq:ch2_stored_coordinate_normalization}所表示的第三章原始坐标变换。
...
=
Q_{0.95}
\left(\{e_i\}_{i=1}^{N_{\mathrm s}}\right),
\label{eq:ch2_mre95}
\end{equation}
其中，$Q_{0.95}$ 表示经验分布的 $0.95$ 分位数。当真实值可能接近零时，不采用逐样本相对误差。对于完整序列，可采用总体相对 $L_1$ 误差
\begin{equation}
e_{\mathrm{rel}}
=
100\%
...
对于真实轮廓点 $\boldsymbol{x}_{i,\ell}^{\mathrm n}$ 和预测轮廓点 $\widehat{\boldsymbol{x}}_{i,\ell}^{\mathrm n}$，平均点误差为
\begin{equation}
e_{\mathrm{pt}}
=
\frac{1}{LN_{\mathrm c}}
...
\sum_{i=1}^{N_{\mathrm c}}
\left\|
\widehat{\boldsymbol{x}}_{i,\ell}^{\mathrm n}
-
\boldsymbol{x}_{i,\ell}^{\mathrm n}
\right\|_2,
...
\end{equation}
其中，$N_{\vartheta}$ 为角度采样点数。几何量 $h\in\{A,L_{\Gamma},\mathcal{C},D_{\mathrm c}\}$ 的平均绝对误差为
\begin{equation}
e_h^{\mathrm{abs}}
=
\frac{1}{L}
...
\label{eq:ch2_geometry_absolute_error}
\end{equation}
面积和周长的平均相对误差为
\begin{equation}
e_h^{\mathrm{rel}}
...
\frac{100\%}{L}
\sum_{\ell=1}^{L}
\frac{|\widehat h_{\ell}-h_{\ell}|}{|h_{\ell}|},
\qquad
h\in\{A,L_{\Gamma}\}.
\label{eq:ch2_geometry_relative_error}
\end{equation}
轮廓重构结果同时报告点坐标误差、整体几何量误差和由重构序列得到的下游参数预测误差。
...
在统一数值框架基础上，本章进一步定义了轮廓点、质心、傅里叶系数、面积、周长、圆度、圆度偏差、质心速度、铺展因子和名义动态接触角，并建立了正问题、观测算子和数据驱动反演之间的数学关系。第三章至第五章将在本章所给方程、符号和评价指标基础上，分别说明具体计算条件、数据构造、网络结构和参数预测结果。
